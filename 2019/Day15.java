...
/**
 * Created by Sun on 12/15/2019.
 * Oxygen System
 */
public class Day15 {

    static int[][] grid; // 0 is unknown, 1 is open, -1 is wall, 2 is target
    static Map<Integer, Integer> minSteps = new HashMap<>();
    ...
    static int part2() {
        ...
        while (true) {
            ...
            for (Integer place : prev) {
                int steps = minSteps.get(place);
                x = place / mod;
                y = place % mod;
                for (int dir = 1; dir <= 4; dir++) {
                    int newX = x + getDiffX(dir);
                    int newY = y + getDiffY(dir);
                    int key = newX * mod + newY;
                    if (grid[newX][newY] != -1 && !minSteps.containsKey(key)) {
                        minSteps.put(key, steps + 1);
                        next.add(key);
                    }
                }
            }
    ...
    static int part1() {
        ...
        printGrid();
        int mod = 1000;
        int x = grid[0].length / 2, y = grid.length / 2;
        minSteps.clear();
        minSteps.put(x * mod + y, 0);
        List<Integer> prev = new ArrayList<>();
        List<Integer> next = new ArrayList<>();
        ...
        while (true) {
            ...
            for (Integer place : prev) {
                ...
                y = place % mod;
                if (grid[x][y] == 2) {
                    System.out.printf("target at %d , %d\n", x, y);
                    return steps;
                }
                for (int dir = 1; dir <= 4; dir++) {
                    int newX = x + getDiffX(dir);
    ...
    static void traverse1(boolean rightHand) {
        Day9.Computer computer = new Day9.Computer(INPUT);
        Day9.Computer.Output output;
        int direction = 1;
        int x = grid[0].length / 2, y = grid.length / 2;
        grid[x][y] = 1;
        boolean foundIt = false;
        while (true) {
            output = computer.compute(Collections.singletonList((long) direction));
            switch (output.output.get(0).intValue()) {
                case 0:
                    grid[x + getDiffX(direction)][y + getDiffY(direction)] = -1;
                    direction = rightHand ? getTurnLeft(direction) : getTurnRight(direction);
                    break;
                case 2:
                    foundIt = true;
                case 1:
                    ...
                    grid[x][y] = foundIt ? 2 : 1;
                    break;
            }
            if (foundIt || output.halted) {
                break;
            }
            if (whatOnMy(x, y, rightHand ? getTurnRight(direction) : getTurnLeft(direction)) != -1) {
                direction = rightHand ? getTurnRight(direction) : getTurnLeft(direction);
            }
        }
    }

    static int getDiffY(int direction) {
        switch (direction) {
            case 1:
                return 1;
            case 2:
    ...
    static int getDiffX(int direction) {
        switch (direction) {
            case 1:
            case 2:
                return 0;
    ...
    static boolean within(int size, int x, int y) {
    ...
    static int getTurnLeft(int direction) {
        switch (direction) {
            case 1:
                return 3;
            case 2:
    ...
    static int getTurnRight(int direction) {
        switch (direction) {
            case 3:
                return 1;
            case 4:
    ...
    static int whatOnMy(int x, int y, int direction) {
        return grid[x + getDiffX(direction)][y + getDiffY(direction)];
    }
