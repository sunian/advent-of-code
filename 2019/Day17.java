...
public class Day17 {
    ...
    static int part2() {
        ...
        String main = "A,B,A,B,C,B,C,A,C,C\n";
        String a = "R,12,L,10,L,10\n";
        String b = "L,6,L,12,R,12,L,4\n";
        String c = "L,12,R,12,L,6\n";
        String code = main + a + b + c + "n\n";
        ...
        return output.output.get(output.output.size() - 1).intValue();
    }

    static String[] getLines() {
        Day9.Computer computer = new Day9.Computer(INPUT);
        Day9.Computer.Output output = computer.compute(Collections.emptyList());
        StringBuilder sb = new StringBuilder(output.output.size());
        for (Long ascii : output.output) {
    ...
    static int part1() {
        ...
        for (int row = 1; row < lines.length - 1; row++) {
            ...
            for (int col = 1; col < line.length() - 1; col++) {
                ...
                if (line.charAt(col - 1) == '.' || line.charAt(col + 1) == '.'
                        || lines[row - 1].charAt(col) == '.' || lines[row + 1].charAt(col) == '.') {
                    continue;
                }
                sum += row * col;
            }
