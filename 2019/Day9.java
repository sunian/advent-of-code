...
public class Day9 {
    ...
    public static class Computer {
        ...
        Output compute(List<Long> newInput) {
            ...
            while (true) {
                long opcode = tape[position];
                int mode1 = (int) ((opcode / 100) % 10);
                int mode2 = (int) ((opcode / 1000) % 10);
                int mode3 = (int) ((opcode / 10000) % 10);
                switch ((int) (opcode % 100)) {
                    case 1: // add
                        long sum = readValue(position + 1, mode1) + readValue(position + 2, mode2);
                        writeValue(tape[position + 3], sum, mode3);
                        position += 4;
                        break;
                    case 2: // multiply
                        long product = readValue(position + 1, mode1) * readValue(position + 2, mode2);
                        writeValue(tape[position + 3], product, mode3);
                        position += 4;
                        break;
                    case 3: // read input
                        ...
                        writeValue(tape[position + 1], input.get(inputPosition), mode1);
                        inputPosition++;
                        position += 2;
                        break;
                    case 4: // write output
                        output.add(readValue(position + 1, mode1));
                        position += 2;
                        break;
                    case 5: // jump-if-true
                        if (readValue(position + 1, mode1) != 0) {
                            position = (int) readValue(position + 2, mode2);
                        } else {
                            position += 3;
                        }
                        break;
                    case 6: // jump-if-false
                        ...
                        break;
                    case 7: // is less than
                        boolean firstIsLess = readValue(position + 1, mode1) < readValue(position + 2, mode2);
                        writeValue(tape[position + 3], firstIsLess ? 1 : 0, mode3);
                        position += 4;
                        break;
                    case 8: // is equal
                        boolean areEqual = readValue(position + 1, mode1) == readValue(position + 2, mode2);
                        writeValue(tape[position + 3], areEqual ? 1 : 0, mode3);
                        position += 4;
                        break;
                    case 9: // relative base
                        relativeBase += readValue(position + 1, mode1);
                        position += 2;
                        break;
                    case 99: // halt
        ...
        private long readValue(int position, int mode) {
            if (mode == 0) {
                return tape[(int) tape[position]];
            } else if (mode == 1) {
                return tape[position];
            } else if (mode == 2) {
                return tape[(int) tape[position] + relativeBase];
            } else {
                throw new IllegalArgumentException("invalid mode: " + mode);
            }
        }

        private void writeValue(long position, long value, int mode) {
            if (mode == 2) {
                tape[(int) position + relativeBase] = value;
            } else if (mode == 0 || mode == 1) {
                tape[(int) position] = value;
            } else {
                throw new IllegalArgumentException("invalid mode: " + mode);
            }
        }
